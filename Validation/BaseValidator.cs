...
abstract class BaseValidator : IValidator
{
	public ValidationResult Validate(Report report) {
		...
		if (result.IsValid)
		{
			ValidationResult result2 = ValidateSpecificFields(report);
		}
	...
	protected ValidationResult ValidateCommonFields(Report report) {
		...
		if (report.Timestamp.CompareTo(minDate) < 0)
			return ValidationResult.Failure($"Error: time of report can not be beefore {minDate}");
		if (report.Latitude > 33.5000 || report.Latitude < 29.5000)
			return ValidationResult.Failure($"Error: Latitude of report shuld be between{1} to {1}");
		if(report.Longitude>36.0000 || report.Longitude<34.0000)
			return ValidationResult.Failure($"Error: Longitude of report shuld be between{1} to {1}");
		if(string.IsNullOrEmpty(report.Description.Trim()) || report.Description.Length>500 || report.Description.Length<100)
			return ValidationResult.Failure($"Error: Description length shuld be between{1} to {1} characters and Description can not br null");
	}
